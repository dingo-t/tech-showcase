...
export default function VoteList() {
  ...
  // Arrays are fetched from App.js using the useContext hook
...
function getVotesForId(id, votes) {
  ...
  return vote ? vote.votes : 0; /* returns amount of votes if 
   no match is found it defaults to 0 */
}
 ...
 <div className='vote-card-container'>
    ...
    {votesData.length > 0 ? (
      <ul className='vote-list-ul'>
        {/* initialcards is iterated through with
         data from each object displayed */}
        {initialCards.map((Card) => (
          <li className='vote-list-li' key={Card.id}>
             <div className='list-text'>{Card.text}</div>
             {/* The votes for the food item object is 
             found using the getvotesforid function*/}
            <div className='list-votes'>Votes: 
            {getVotesForId(Card.id, votesData)}</div>
          </li>
        ))}
      </ul>
    ) : null}
