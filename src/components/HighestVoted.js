...
export default function HighestVoted() {
    ...
    let CardData = undefined;
     ...
     When the voteData has had its data fetched from the database using PHP the component will rerender and run the rest of the code */
    ...
    // Once the highest votes have been found this function matches the vote value with information of the food item from the cards array in App.js
    function getValuesFromArrays(votes, cards) {
        // returned value is declared as an empty array
        const result = [];
        // the id of each object in the votes array is matched with the cororesponding object from the cards array
        votes.forEach(vote => {
          const matchingCard = cards.find(card => card.id === vote.id);
      
        // These objects are merged which forms a new array with vote values and data from the cards array
          if (matchingCard) {
            result.push({
              id: vote.id,
              votes: vote.votes,
      ...
      return (
        <div className='highest-voted-card-cont'>
          ...
          {/* If the CardData array has not yet been defined text saying the vote data has not been fetched is shown
              this prevents errors from the code trying to use an array that is undefined */}
            {typeof CardData !== 'undefined' ? (
              // the array is iterated over with the values of each object used to show the name, vote and an image of the food item
              CardData.map(item => (          
