...
// The function is a panel that allows users to vote for a 
// food item and to view the amount of votes it has
function VotingPanel(props) {
  /* The function passes in an id to a php file when the vote button is click
   This causes the vote item with the corresponding id 
   to have its vote count incremented */
  const handleButtonClick = async () => {
   
    const id = props.id;
    
    // The php file is run and the id is passed using the get 
    // method since it is not sensitive information
    try {
      const response = await fetch(`http://localhost/UpdateVote.php?id=${id}`, { 
        method: 'GET', 
      });

      /* If the response is successful the result is logged
         and UserVoted is set to true in 
         ocal storage to prevent users from voting twice */
      if (response.ok) {
        const result = await response.text();
        localStorage.setItem('UserVoted', true);  
        // rerenders component so that updated votes are 
        // displayed and vote button is removed
...
<div className='voting-cont'>
  ...
  <div className='vote-button-section'>
    {/* if the user has voted the vote button is hidden and a message 
        thanking the user for voting is rendered */}
    {HasVoted ? (
